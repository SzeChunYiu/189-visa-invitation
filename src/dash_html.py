...
function chartWaterfall(g,gk,pts){
  ...
  /* A true waterfall: the deductions float between the start and the end, so the
     three drops are the subject. Four full-height bars all near 90% showed the
     levels and hid the differences, which are the whole point of the panel. */
  const W=560,H=232,ML=20,MR=20,MT=46,MB=64,pw=W-ML-MR,ph=H-MT-MB;
  frame(s,W,H);
  if(!g||g.share<=0) return;
  ...
  const cols=[
    {kind:"total", v:le,  lab:"cut-off reaches "+pts, sub:"at this round size"},
    {kind:"drop", from:le,   to:atSize, lab:"boundary band", sub:sh===null?"no date given":"date of effect"},
    {kind:"drop", from:atSize, to:cond, lab:"round size",    sub:"small rounds cut deeper"},
    {kind:"drop", from:cond, to:fin,    lab:"group skipped", sub:Math.round(z*100)+"% of the time"},
    {kind:"total", v:fin, lab:"your chance", sub:"if a round is held"}];
  const bw=pw/cols.length, Y=v=>MT+ph*(1-v);
  for(let q=0;q<=1.0001;q+=0.25){
    s.appendChild(el("line",{x1:ML,y1:Y(q),x2:ML+pw,y2:Y(q),class:"gl"}));}
  let prevX=null, prevY=null;
  cols.forEach((c,i)=>{
    const x=ML+i*bw+bw*0.17, bwid=bw*0.66;
    let top,hgt,fill,op;
    ...
    const val = c.kind==="total" ? Math.round(c.v*100)+"%"
                                 : "−"+Math.round((c.from-c.to)*100)+"pp";
    const t=el("text",{x:x+bwid/2,y:top-7,class:"vlab","text-anchor":"middle",
      fill:c.kind==="total"?"var(--ink)":"var(--crit)"});
    t.textContent=val;s.appendChild(t);
    hover(s.lastChild,"");
    const l1=el("text",{x:x+bwid/2,y:H-MB+18,class:"tick","text-anchor":"middle","font-weight":"700",
      fill:"var(--ink)"});l1.textContent=c.lab;s.appendChild(l1);
    const l2=el("text",{x:x+bwid/2,y:H-MB+31,class:"tick","text-anchor":"middle"});
    l2.textContent=c.sub;s.appendChild(l2);
    /* the connector carries the eye from one step to the next */
  ...
  const yl=el("text",{x:ML,y:MT-26,class:"tick","text-anchor":"start","font-weight":"700",
    fill:"var(--ink)"});
  yl.textContent="Three deductions separate the cut-off reaching you from being invited";
  s.appendChild(yl);
