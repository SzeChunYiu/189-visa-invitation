...
<div class="card"><div class="chead"><h2>Your chance, against how many they invite</h2>
  <span class="eyebrow" id="h8n"></span><button class="q" type="button" aria-expanded="false" aria-controls="n1" aria-label="Explain this panel" data-note="n1">?</button></div>
...
<div class="grid2">
  <div class="card"><div class="chead"><h2>Would you have got in before?</h2>
    <span class="eyebrow" id="h1n"></span><button class="q" type="button" aria-expanded="false" aria-controls="n2" aria-label="Explain this panel" data-note="n2">?</button></div>
    ...
    <span class="eyebrow" id="h4n"></span><button class="q" type="button" aria-expanded="false" aria-controls="n3" aria-label="Explain this panel" data-note="n3">?</button></div>
...
<div class="card"><div class="chead"><h2>Every score band, and who is ahead of you</h2>
  <span class="eyebrow" id="hbn"></span><button class="q" type="button" aria-expanded="false" aria-controls="n4" aria-label="Explain this panel" data-note="n4">?</button></div>
  <div class="scroll"><table id="bt"><thead><tr><th>Points</th><th>In your occupation</th>
    <th>In your unit group</th><th>Running total ahead</th><th>Standing vs you</th>
...
<div class="grid2">
  ...
  <div class="card"><div class="chead"><h2>Why some occupations need more points</h2>
    <span class="eyebrow">Jun 2026 round</span><button class="q" type="button" aria-expanded="false" aria-controls="n7" aria-label="Explain this panel" data-note="n7">?</button></div>
...
<div class="card"><div class="chead"><h2>Round by round</h2><span class="eyebrow">the record for this occupation</span><button class="q" type="button" aria-expanded="false" aria-controls="n8" aria-label="Explain this panel" data-note="n8">?</button></div>
...
<div class="card"><div class="chead"><h2>Every occupation at your score</h2>
  ...
  </span><button class="q" type="button" aria-expanded="false" aria-controls="n9" aria-label="Explain this panel" data-note="n9">?</button></div>
...
/* ---------- one table: every band, the running head count, and the chance ---------- */
function bandTable(o,g,pts){
  ...
  keys.forEach(k=>{
    const gc=g.dist[k]||0;
    if(k>=my) run+=gc;
    const saveP=S.pts; S.pts=k; const P=pMarginal(g,k); S.pts=saveP;
    const stand = k>my ? ["crit","ahead on points"] : k===my ? ["warn","ahead on date"] : ["n","behind you"];
    const tr=document.createElement("tr"); if(k===my)tr.className="hl";
    [k+(k===my?" (you)":""),fmt(o.dist[k]||0),fmt(gc),k>=my?fmt(run):"—","",""].forEach((cell,ci)=>{
      const td=document.createElement("td");
      if(ci===4){const sp=document.createElement("span");sp.className="pill "+stand[0];
        sp.textContent=stand[1];td.appendChild(sp);}
      else if(ci===5){const sp=document.createElement("span");
        sp.className="pill "+(P===null?"n":P>=.8?"good":P>=.5?"warn":"crit");
        sp.textContent=P===null?"—":Math.round(P*100)+"%";td.appendChild(sp);}
      else td.textContent=cell;
      tr.appendChild(td);});
    tb.appendChild(tr);});
  const al=g.alloc[g.alloc.length-1];
  const tr=document.createElement("tr");
  const a=document.createElement("td");a.colSpan=3;a.innerHTML="<b>Ahead of you in "+o.g+"</b>";
  const b=document.createElement("td");b.innerHTML="<b>"+fmt(run)+"</b>";
  const c=document.createElement("td");c.colSpan=2;
  const sp=document.createElement("span");sp.className="pill "+(al>=run?"good":"crit");
  sp.textContent=(al>=run?"last round reached ":"last round stopped at ")+fmt(al);
  c.appendChild(sp);
