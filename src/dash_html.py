...
function chartRoundSize(){
  ...
  const W=560,H=214,ML=20,MR=20,MT=44,MB=52,pw=W-ML-MR,ph=H-MT-MB;
  frame(s,W,H);
  if(!B.rs) return;
  ...
  const hd=el("text",{x:ML,y:MT-26,class:"tick","text-anchor":"start","font-weight":"700",fill:"var(--ink)"});
  hd.textContent="Nobody knows how large the next round will be";s.appendChild(hd);
  const hd2=el("text",{x:ML,y:MT-13,class:"tick","text-anchor":"start"});
  hd2.textContent=fmt(B.policy.places["2026-27"])+" places × "+B.policy.ratio+
    " invitations per place, spread over the rounds still expected";
  s.appendChild(hd2);
...
function chartGroupShare(g,gk){
  ...
  const W=560,H=214,ML=52,MR=64,MT=44,MB=52,pw=W-ML-MR,ph=H-MT-MB;
  frame(s,W,H);
  if(!g) return;
  ...
  [grid[0],grid[grid.length-1]].forEach((N,i)=>{
    ...
    q.textContent=fmt(N);s.appendChild(q);});
  const hd=el("text",{x:ML,y:MT-26,class:"tick","text-anchor":"start","font-weight":"700",fill:"var(--ink)"});
  hd.textContent=g.name;s.appendChild(hd);
  const hd2=el("text",{x:ML,y:MT-13,class:"tick","text-anchor":"start"});
  hd2.textContent="took "+fmt(g.alloc[g.alloc.length-1])+" last round — a "+
    (g.share*100).toFixed(2)+"% share of everything issued";
  s.appendChild(hd2);
  const h=$("h12n"); if(h) h.textContent="places, by round size";
