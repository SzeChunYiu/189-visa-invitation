...
function chartRounds(o,pts){
  ...
  if(fcN!==null&&fcN!==undefined){
    ...
    if(pIn!==null){
      ...
      t1.textContent="cut-off reaches "+pts+" in "+Math.round(pIn*100)+"% of rounds this size";
      s.appendChild(t1);}
...
function chartProb(g,pts){
  ...
  /* where the curve crosses even odds - the one number the text box carried */
  {
    ...
    for(let v=S0;v<=S1;v+=100){const q=pClear(cutoffAt(g,v),pts);
